...
static struct workqueue_struct *wq; //分配工作队列指针
...
static irqreturn_t button_isr(int irq, void *dev_id)
{
    ...
    //登记工作，将自己的工作交给自己创建的内核线程和自己创建的工作队列去处理
    //queue_work(wq, &work);
    
    //登记延时工作，将自己的延时工作交给自己创建的内核线程和工作队列去处理
...
static int btn_init(void)
{
    ...
    //创建自己的工作队列和内核线程
    //wq记录创建的工作队列
    //zhangsan:创建的内核线程名，并且将工作队列和内核线程进行绑定
    wq = create_workqueue("zhangsan");
    return 0;
}
...
static void btn_exit(void)
{
    ...
    destroy_workqueue(wq); //销毁工作队列和内核线程
}
